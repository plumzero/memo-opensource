...
int main(int argc, char* argv[])
{
  ...
  if (mode == 1) {
    ...
    for (long offset = 0; offset < fsize; offset += rlen) {
      ...
      // 如果不是最后一段，则不作填充
      if (offset + n < fsize) {
        len = RSA_public_encrypt(n, inbuf, outbuf, r, RSA_NO_PADDING);
        if (len <= 0) {
          printf("RSA_public_encrypt(RSA_NO_PADDING) err!\n");
          return -1;
        }
      } else if (offset + n == fsize) {
        len = RSA_public_encrypt(n, inbuf, outbuf, r, padding);
        if (len <= 0) {
          printf("RSA_public_encrypt(RSA_PKCS1_PADDING) err!\n");
          return -1;
        }
      }
  ...
  if (mode == 2) {
    ...
    for (long offset = 0; offset < fsize; offset += rlen) {
      ...
      int len;
      if (offset + n < fsize) {
        len = RSA_private_decrypt(n, inbuf, outbuf, r, RSA_NO_PADDING);
        if (len <= 0) {
          printf("RSA_private_decrypt(RSA_NO_PADDING) err!\n");
          return -1;
        }
      } else if (offset + n == fsize) {
        len = RSA_private_decrypt(n, inbuf, outbuf, r, padding);
        if (len <= 0) {
          printf("RSA_private_decrypt(RSA_PKCS1_PADDING) err!\n");
          return -1;
        }
      }
      if (fwrite(outbuf, 1, len, fout) != (size_t)len) {
