...
int main()
{
  ...
  const char* issuer_path = "issuerca.pem";
  X509* issuer = pem_to_x509(issuer_path);
  ret = X509_STORE_add_cert(cert_store, issuer);
  if (ret != 1) {
    printf("X509_STORE_add_cert err!\n");
    return -1;
  }
  ...
  /* 将待检验证书加入存储区 */
  const char* user_path = "encoded.pem";
