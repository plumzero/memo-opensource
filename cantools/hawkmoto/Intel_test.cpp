...
/*
  ...
  上面的示例中，使用的是 Intel 格式，这种格式是从低字节低位起始。接下来分析在这种格式下是如何对信号值进行 pack 的。
  ...
  (0,0) 是起始点位，当它作为这个信号值的最低位时，第一个字节中仍有比它更高的比特位。
  ...
  (1, 4) 是起始点位，当它作为这个信号值的最低位时，第二个字节仍有比它更高的空闲比特位。AverageRadius 最终所占空间点位如下:
      7   6   5   4   3   2   1   0
    +---+---+---+---+---+---+---+---+
  0 | 0 | 0 | 1 | 1 | 0 | 1 | 1 | 1 |
    +---+---+---+---+---+---+---+---+
